...
{
    public partial class FrmPurchaseDrNote : Form
    {
     ...
     (
        ...
        public FrmPurchaseDrNote()
        {
            ...
            btnSelectDrNote.Region = System.Drawing.Region.FromHrgn(CreateRoundRectRgn(0, 0, btnSelectDrNote.Width, btnSelectDrNote.Height, 5, 5));
        ...
        private void CalculateTotalSummaryToGrid()
        {
            try
            {
                ...
                foreach (DataRow item in StockTranGridTbl.Rows)
                {
                    if (ADD_NEW_BOOL)
                    {
                        Qty = Convert.ToDecimal(item["QtyIn"]);// data from stktran and TranType = Invoice where qty is in QtyIn
                    }
                    else Qty = Convert.ToDecimal(item["QtyOut"]);// data from stktran and TranType = Returns where qty is in QtyOut

                    Dictionary<string, decimal> PurchaseGSTlist = MdlMain.PurchaseGSTCalculation(Convert.ToDecimal(item["StkCost"]), Qty, Convert.ToDecimal(item["GST"]), Convert.ToDecimal(item["Cess"]), Convert.ToDecimal(item["DiscountPer"]), Convert.ToDecimal(item["Discount"]));
                ...
                summRow["TotCountDescription"] = "Total Items: " + StockTranGridTbl.Rows.Count;
        ...
        private void SetPurReturnSummaryDetails(int purRetNo)
        {
            try
            {
                var retList = (from pRetMst in cmpDBContext.PurchaseRetMaster
                               join vnd in cmpDBContext.Vendor on pRetMst.VendID equals vnd.VendorId
                               where pRetMst.PurInvNo == purRetNo
                               select new
                               {
                                   pRetMst.InvDate,
                                   pRetMst.RetDate,
                                   ...
                                   pRetMst.RoundAmount,
                                   pRetMst.InvRef,
                                   pRetMst.TotalCessAmount,
                                   pRetMst.TotalCGSTAmount,
                                   pRetMst.TotalSGSTAmount,
                                   pRetMst.TotalIGSTAmount,
                                   pRetMst.AddlCess,
                                   pRetMst.AddlDiscount,
                                   pRetMst.SubTotal,
                                   vnd.VendorName,
                                   vnd.VendorId,
                                   vnd.VendorAdd1,
                ...
                {
                    PurchaseDrNoteNo = purRetNo;
                    PurchaseInvoiceNo = purRetNo;
                    PurchasePayNo = inv.PayNo;
                    VendorId = inv.VendorId;
                    lblVendorName.Text = inv.VendorName.Trim();
                    lblAddress.Text = inv.VendorAdd1 + ", " + inv.VendorAdd2;
        ...
        private void SetPurRetGridDetails(int purRetNo)
        {
            try
            {
                StockTranGridTbl.Clear();
                var stkTranList = (from stkTran in cmpDBContext.StkTrans
                                   join stk in cmpDBContext.Stock on stkTran.StocksId equals stk.StockId
                                   join bth in cmpDBContext.Batch on stkTran.BatchId equals bth.BatchId
                                   where stkTran.InvNo == purRetNo && stkTran.TranRef == "Returns" && stkTran.TranType == "Purchase"
                                   select new
                                   {
                                       stkTran.TranId,
                                       stkTran.InvNo,
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (FieldValidation())
            {
                if (ADD_NEW_BOOL == false && PurchaseDrNoteNo > 0)
                {
                    List<StkTran> stkTr = cmpDBContext.StkTrans.Where(m => m.InvNo == PurchaseDrNoteNo && m.TranType == "Purchase" && m.TranRef == "Returns").ToList();
                    if (stkTr.Count > 0)
                    {
                        cmpDBContext.StkTrans.RemoveRange(stkTr);
                        cmpDBContext.SaveChanges();
                    }
                    IsGlbreakExist(PurchaseDrNoteNo.ToString(), TxtVendRef.Text.Trim());
                }
                decimal paidAmount = TxtPaidAmount.Text != "" ? Convert.ToDecimal(TxtPaidAmount.Text) : Convert.ToDecimal(0);
                SaveRetPurchaseInvMaster();
                SaveStock();
                SaveBatch();
                SaveStkRetTran();
                SetRetGlBreak();
                ...
                if (ADD_NEW_BOOL)
                {
                    MessageBox.Show("Purchase Return saved successfully!!!", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else MessageBox.Show("Purchase Return updated successfully!!!", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private bool SaveRetPurchaseInvMaster()
        {
            try
            {
                if (PurchaseDrNoteNo > 0)
                {
                    List<PurchaseRetMaster> pRetMaster = cmpDBContext.PurchaseRetMaster.Where(m => m.PurRetNo == PurchaseDrNoteNo).ToList();
                    foreach (PurchaseRetMaster pinv in pRetMaster)
                    {
                        foreach (DataRow item in InvSummTbl.Rows)
                        {
                            pinv.InvDate = DtpDrNoteDt.Value.Date;
                            pinv.PurInvNo = MdlMain.gPurchaseInvNo;
                            pinv.RetDate = DtpDrNoteDt.Value.Date;
                            ...
                            pinv.TotDiscount = Convert.ToDecimal(item["DiscountTotal"]);
                            ...
                            pinv.TotalCessAmount = Convert.ToDecimal(item["CessTotal"]);
                            pinv.TotalCGSTAmount = Convert.ToDecimal(item["CGSTTotal"]);
                            pinv.TotalSGSTAmount = Convert.ToDecimal(item["SGSTTotal"]);
                            pinv.TotalIGSTAmount = Convert.ToDecimal(item["IGSTTotal"]);
                            pinv.AddlCess = Convert.ToDecimal(item["AddlCessTotal"]);
                            pinv.AddlDiscount = 0;
                            pinv.PayBankIdNo = BankID;
                            pinv.PayNo = PurchasePayNo;
                            pinv.BillType = "GST";
                ...
                else
                {
                    foreach (DataRow item in InvSummTbl.Rows)
                    {
                        var purchaseRetMasters = new PurchaseRetMaster()
                        {
                            RetDate = DtpDrNoteDt.Value.Date,
                            InvDate = DtpTranDate.Value.Date,
                            PurInvNo = MdlMain.gPurchaseInvNo,
                            VendID = VendorId,
                            ...
                            AddlCess = Convert.ToDecimal(item["AddlCessTotal"]),
                            AddlDiscount = 0,
                            PayBankIdNo = BankID,
                            PayNo = PurchasePayNo,
                            BillType = "GST",
                        ...
                        cmpDBContext.PurchaseRetMaster.Add(purchaseRetMasters);
                        cmpDBContext.SaveChanges();
                        PurchaseDrNoteNo = purchaseRetMasters.PurRetNo;
                    }
            ...
            try
            {
                foreach (DataRow item in StockTranGridTbl.Rows)
                {
                    ...
                    var stkTran = new StkTran()
                    {
                        ...
                        InvNo = PurchaseDrNoteNo,
                        TranDate = DtpDrNoteDt.Value.Date,
                        TranRef = "Returns",
                        TranType = "Purchase",
            ...
            try
            {
                foreach (DataRow item in InvSummTbl.Rows)
                {
                    ...
                    if (Convert.ToDecimal(TxtGrandTotal.Text) > 0)
                    {
                        ...
                        {
                            TranRef = PurchaseDrNoteNo.ToString(),
                            TranDate = DtpDrNoteDt.Value.Date,
        ...
        private void IsGlbreakExist(string InvNo, string VendRef)
        {
            try
            {
                List<GlBreak> vbreak = cmpDBContext.GlBreaks.Where(m => m.TranRef == InvNo && m.LedAc == VendRef && m.GlModule == "Purchase" && m.TranType == "Returns").ToList();
                if (vbreak.Count > 0)
                {
        ...
        private void btnSelectDrNote_Click(object sender, EventArgs e)
        {
        ...
        private void GetSalesCreditNoteList()
        {
            try
            {
                FrmPurchaseRetSelectList frmPurchaseRetSelectList = new FrmPurchaseRetSelectList();
                frmPurchaseRetSelectList.ShowDialog();
                if (MdlMain.gPurchaseDrNoteNo > 0)
                {
                    InitializingData();
                    ADD_NEW_BOOL = false;
                    PurchaseDrNoteNo = MdlMain.gPurchaseDrNoteNo;
                    MdlMain.gSalesCrNoteNo = 0;
                    SetPurReturnSummaryDetails(PurchaseDrNoteNo);
                    SetPurRetGridDetails(PurchaseDrNoteNo);
                    //SetBankDetails(BankID);                   
                    foreach (DataRow item in InvSummTbl.Rows)
                    {
